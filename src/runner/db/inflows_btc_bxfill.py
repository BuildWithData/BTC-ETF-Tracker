...
inflows = pd.DataFrame(c.execute(QUERY), columns=["ref_date"] + TICKERS + ["TOTAL"])

QUERY = "select * from holdings_btc_bfill"
holdings = pd.DataFrame(c.execute(QUERY), columns=["ref_date"] + TICKERS + ["TOTAL"])
...
out.iloc[2, -1] = NaN
out.iloc[27, -1] = NaN
out = out[["ref_date"] + [t for t in TICKERS if t != "BTCO"] + ["TOTAL"]]
